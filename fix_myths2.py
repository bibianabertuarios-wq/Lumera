...
lines_after = open('app/lumera/LumeraApp.jsx', 'r').readlines()

# recalcular tips después del reemplazo
f2 = open('app/lumera/LumeraApp.jsx', 'w')
f2.writelines(lines[:5501] + [new_myths] + lines[5546:])
f2.close()

f3 = open('app/lumera/LumeraApp.jsx', 'r')
lines2 = f3.readlines()
f3.close()
...
for i, l in enumerate(lines2):
    if 'key={`tips-${language}`}' in l and start_tips is None:
        start_tips = i
    if start_tips and end_tips is None and '};' in l and i > start_tips + 15:
        end_tips = i + 1
        break

print(f"tips: {start_tips+1}-{end_tips}")
lines2[start_tips:end_tips] = [new_tips]

f4 = open('app/lumera/LumeraApp.jsx', 'w')
f4.writelines(lines2)
f4.close()
print("OK mitos y consejos")
